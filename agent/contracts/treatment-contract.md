...
`compositionMode` never weakens `seamless-default`. A `travel` rationale names a concrete honest spatial relationship; otherwise use `hold`. `chapterCutBudget: 1` is permission to consider at most one honest cut, not a requirement or a waiver of cut evidence. The Treatment contains no pixels, exact frames, coordinates, React/CSS, capability implementation, audio plan, review, or approval.
